...
for d in ./*; do
	...
	if [[ "$d" != "./README.md"  && "$d" != "./atualizar.sh" && "$d" != "./nova_aula.sh" ]]; then
		...
		echo "## [$d](https://github.com/IgorAvilaPereira/"$repositorio"/tree/main/$d) <br>" >> "./../$1.wiki/Home.md"		
		cd $d							
		...
		for d2 in *.*; do
			...
			# se n tiver wiki, favor comentar
			echo "[$d2](https://github.com/IgorAvilaPereira/"$repositorio"/blob/main/$d/$d2) <br>" >> "./../../$1.wiki/Home.md"					
		done
		...
		for i in */; do 			
			if [[ -d "$i" && ! -L "$i" ]]; then
				...
				# se n tiver wiki, favor comentar
				echo "[${i%%/}](https://github.com/IgorAvilaPereira/"$repositorio"/blob/main/$d/${i%%/}) <br>" >> "./../../$1.wiki/Home.md"	
			fi
		done
		cd ..
	fi
